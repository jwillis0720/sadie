...
class CustomConverter:
    ...
    def _parse_gene_info(self, gene_name: str) -> Dict[str, str]:
        ...
        if match:
            return {"chain": match.group(1), "segment": match.group(2)}
    ...
    def convert(
        ...
        organized: Dict[str, Dict[str, List[Dict]]] = defaultdict(lambda: defaultdict(list))
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Convert G3 custom JSON to FASTA files")
    parser.add_argument("--input", "-i", type=Path, required=True, help="Input JSON file (G3.custom.json)")
    parser.add_argument("--output-dir", "-o", type=Path, default=None, help="Output directory for FASTA files")
    parser.add_argument("--species", nargs="+", default=None, help="Only process these species")
    parser.add_argument("--list-species", action="store_true", help="List all species in JSON and exit")
    parser.add_argument("--include-gapped", action="store_true", help="Also write gapped FASTA files (for validation)")
    ...
    try:
        ...
        results = converter.convert(
            args.input, species_filter=args.species, include_gapped=args.include_gapped, force=args.force
        )
