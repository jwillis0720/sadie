...
class OrganismYamlUpdater:
    ...
    def update(self, species: Optional[List[str]] = None, force: bool = False) -> Dict:
        """
        Update organism.yaml with species configurations.
        ...
        for sp, info in species_to_update.items():
            ...
            entry = {"database_path": info["database_path"], "segments": info["segments"]}
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Update organism.yaml with species configurations")
    parser.add_argument("--species", nargs="+", help="Species to add/update")
    parser.add_argument("--all-species", action="store_true", help="Update all species with available databases")
    parser.add_argument("--force", action="store_true", help="Force overwrite existing entries")
    parser.add_argument("--list-species", action="store_true", help="List available species and their status")
