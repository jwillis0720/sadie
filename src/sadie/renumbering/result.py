...
class NumberingResults(pd.DataFrame):
    ...
    def _add_segment_regions(self) -> "NumberingResults":
        ...
        segmented_df["leader"] = segmented_df[["sequence", "seqstart_index"]].apply(lambda x: x.iloc[0][: x.iloc[1]], axis=1)

        # everything following the antibody. keyword tail will clash with pandas
        segmented_df["follow"] = segmented_df[["sequence", "seqend_index"]].apply(lambda x: x.iloc[0][x.iloc[1] + 1 :], axis=1)
        return segmented_df
