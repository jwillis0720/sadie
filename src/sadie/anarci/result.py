...
class AnarciResult:
    ...
    def __init__(self, summary: pd.Series, alignment: pd.DataFrame):
    ...
    @summary.setter
    def summary(self, summary: pd.Series):
        if not isinstance(summary, pd.Series):
            raise TypeError(f"{summary} must be of type {pd.Series}")
    ...
    def serialize(self):
        return {
            "alignment": json.loads(self.alignment.to_json(orient="records")),
